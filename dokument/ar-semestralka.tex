...
P_{S}\left ( s \right )=F_{c}\left ( s \right )\cdot P_{0}\left ( s \right )=\frac{0.002727}{0.5 s^{3} + 1.114 s^{2} + 0.231 s + 0.0062}.
\end{equation}
...
C\left ( s \right )=K\cdot \left ( 1+\frac{1}{T_{i}s} \right ) = \frac{83.25s+10.9023}{s}.
\end{equation}
...
L_{0}=CP_{S}=\frac{xxxx}{xxxxx}.
\end{equation}
Nyní zjistěme počet nestabilních pólů $ P $ a nestabilních nul $ Z $ a definujme:
\begin{equation}\label{eq:N=P-Z} 
N=P-Z.
\end{equation}
Z přenosu zjistíme, že $ P=1 $ a $ Z=0 $, tedy $ N=1 $. Nyquistovo kritérium stability říká, že 

Vykresleme tedy Nyquistovu křivku pro otevřený systém na obrázku.
...
\begin{equation}
1+L_{0}=1+CP_{S}=\frac{xxxx}{xxxxx}.
\end{equation}
